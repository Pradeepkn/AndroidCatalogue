...
public class ImageCache {
    ...
    private static final int DEFAULT_MEM_CACHE_SIZE = 1024 * 50; // 5MB
    ...
    private LruCache<String, BitmapDrawable> mMemoryCacheDrawable;
...
//        int memCacheSize = calculateMemCacheSize(0.6f);
...
//                }
            ...
            mMemoryCacheDrawable = new LruCache<String, BitmapDrawable>(mCacheParams.memCacheSize) {
...
//                }
     ...
     */
    public void addBitmapToCacheDrawable(String data, BitmapDrawable value) {
        if (data == null || value == null) {
            return;
        }

        // Add to memory cache
        if (mMemoryCacheDrawable != null) {
            if (RecyclingBitmapDrawable.class.isInstance(value)) {
                // The removed entry is a recycling drawable, so notify it
                // that it has been added into the memory cache
                ((RecyclingBitmapDrawable) value).setIsCached(true);
            }
            mMemoryCacheDrawable.put(data, value);
        }
     ...
     */
    public BitmapDrawable getBitmapFromMemCacheDrawable(String data) {
        BitmapDrawable memValue = null;

        if (mMemoryCacheDrawable != null) {
            memValue = mMemoryCacheDrawable.get(data);
        }
    ...
    public static class ImageCacheParams {
         ...
         * Throws {@link IllegalArgumentException} if percent is < 0.05 or > .8. memCacheSize is
         * stored in kilobytes instead of bytes as this will eventually be passed to construct a
         * LruCache which takes an int in its constructor. This value should be chosen carefully
         * based on a number of factors Refer to the corresponding Android Training class for more
